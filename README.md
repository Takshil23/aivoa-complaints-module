...
## 0. How this maps to the assignment

| The brief asks for | Where it is |
|---|---|
| React + Redux | `frontend/src/store/` — two slices, `Provider` in `main.jsx`; the form renders `state.complaint.sections` |
| Python + FastAPI | `backend/app/main.py`, routers in `backend/app/api/` |
| LangGraph | `backend/app/agent/graph.py` — router node, one node per tool, `finalize` |
| Groq `gemma2-9b-it` | `PRIMARY_MODEL` in `backend/.env`; still requested first — see the availability note in §1 |
| `llama-3.3-70b-versatile` "for context" | the router node, for native tool calling |
| MySQL / PostgreSQL | `DATABASE_URL`; schema proven with `scripts/verify_db.py` |
| Google Inter | `frontend/index.html`, applied in `styles.css` |
| Log Complaint tool | `log_complaint` in `backend/app/agent/tools.py` |
| Edit Complaint tool | `edit_complaint` — returns a sparse patch |
| Document Extraction tool | `extract_document` + `services/document_parser.py` |
| Own sample complaint documents | `samples/` — two PDFs and an email, generated by `samples/generate_samples.py` |
| Bonus: Completeness Checker | `/api/ai/completeness` |
| Bonus: Root Cause Recommendation | `/api/ai/root-cause` |
| Bonus: CAPA Recommendation | `/api/ai/capa` |
| Bonus: Duplicate Detection | `/api/ai/duplicates` — a database query, not an LLM call |
| Bonus: Complaint Summary | `/api/ai/summary` |
| Bonus: AI Risk Classification | built into all three tools, so a complaint cannot exist without one |
| Demo video | script in [docs/DEMO-SCRIPT.md](docs/DEMO-SCRIPT.md) |
| Explain the code at interview | [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) is written as that walkthrough |
...
The most instructive part was the first run against a real Groq key. Everything
until then had exercised a deterministic fallback extractor, so the prompts had
never met a model — and six things broke at once. The model returned my prompt
instructions as the officer-facing reply; replacing those instructions with
realistic examples was *worse*, because it then transcribed the example record
into a live complaint and dropped a field; severity refused to escalate for
foreign matter in an API; the run-on batch number came back whole however firmly
the prompt asked; the extraction reply narrated itself twice; and an enum
placeholder came back as the option list. They are catalogued in
[CLAUDE.md](CLAUDE.md), and the general rule that came out of it — never put an
instruction, or a list of options, where a value belongs in a JSON example — is
the kind of thing you only learn by running it.
